...
const (
	...
	targetWordCount  = 225 // Aim for around 200-250 words per paragraph
...
func worker(fileChan <-chan string, wg *sync.WaitGroup, failedFiles *[]string, tokenizer *english.NewSentenceTokenizer, isRetry bool) {
	defer wg.Done()
...
func extractDocxText(path string) string {
	cmd := exec.Command("unioffice", "extract", path)
	var out bytes.Buffer
	cmd.Stdout = &out
	if err := cmd.Run(); err != nil {
...
func chunkText(text, base string, tokenizer *english.NewSentenceTokenizer) []string {
	var paragraphs []string
	var currentParagraph strings.Builder
	var currentWordCount int
	var sentenceBuffer []string
	targetWordCount := 225 // Aim for around 200-250 words
	overlapSentenceCount := 3 // Overlap by the last 3 sentences within a paragraph
...
func formatText(text string) string {
	...
	for i := 0; i < len(words); i += wordsPerSection {
		...
		formatted.WriteString(fmt.Sprintf("%d. ", sectionNum))
		sectionNum++
		...
		for j := i; j < end; j++ {
			...
			if firstLine {
				...
				if currentLineLength+len(word) <= lineLength-3 { // -3 for section number and dot
